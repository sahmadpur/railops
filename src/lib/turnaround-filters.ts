...
import { locomotives, turnarounds } from "@/db/schema";
 ...
 * always matches the screen. Callers must join locomotives (both already do).
 */
export function turnaroundFilters(input: TurnaroundFilterInput): SQL[] {
  ...
  if (input.q) {
    filters.push(or(ilike(turnarounds.trainNumber, `%${input.q}%`), ilike(locomotives.number, `%${input.q}%`))!);
  }
  return filters;
}
